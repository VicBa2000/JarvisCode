...
# >>> LO QUE DEVUELVE PREGUNTAR POR LA CARPETA, Y SON TRES COSAS <<<
# No se puede contestar con `Path | None`: "canceló" y "no se pudo
# preguntar" son estados distintos y piden mensajes distintos. Al primero
# hay que callarse -- ya dijo que no --; al segundo hay que explicarle
# como hacerlo a mano. Colapsarlos deja a alguien que cerró el selector
# leyendo un parrafo sobre lineas de ordenes que no pidio.
ELEGIDA, CANCELO, NO_SE_PUDO = "elegida", "cancelo", "no_se_pudo"


def pedir_carpeta() -> tuple[str, Path | None]:
    """Abre el selector de carpetas de Windows. Ver las tres salidas.

    >>> POR QUE ESTO EXISTE (2026-09-10) <<<
    Lo pregunto el usuario mirando su propio Escritorio: *"hay uno en mi
    desktop y no se si jale"*. No jalaba, y lo que encontramos al medirlo
    es que **le pasa igual a cualquiera que clone el repositorio**: en una
    configuracion recien sembrada `sesion.carpeta` es `""`, y ni
    `jarvis.bat` ni el acceso directo pasan carpeta. O sea que el primer
    doble clic de un usuario nuevo terminaba en un cuadro de "no ha
    podido arrancar", justo despues de que el README le prometiera que
    con dos dobles clics estaba.

    El mensaje que salia no era malo -- decia que hacer --, pero le
    contestaba "arranca una vez pasandola detras del comando" a alguien
    que acababa de hacer DOBLE CLIC, o sea que no tiene ninguna terminal
    delante. Es media leccion aprendida: el 2026-09-05 ya se corrigio
    para que no mandara a editar un YAML, y se quedo mandando a una
    consola.

    >>> Y NO SE ELIGE UNA CARPETA POR DEFECTO, QUE ERA LO FACIL <<<
    Acotar aqui la sesion a "Documentos" o a la carpeta del usuario seria
    apuntar un agente con la autoridad entera de quien lo instala a un
    sitio que no ha elegido nadie. La carpeta la elige una persona, y
    este es el momento en el que esta delante.

    Se usa `tkinter` -- que viene con Python -- y no `SHBrowseForFolderW`
    por `ctypes`, pero con la misma red que `avisar_de_que_no_arranco`:
    si no se puede ni preguntar, se dice y se cae al camino de antes. Un
    selector que revienta no puede dejar el arranque sin explicacion.
    """
    try:
        import tkinter
        from tkinter import filedialog
    except Exception:  # noqa: BLE001 - sin tk no hay nada que ofrecer
        return NO_SE_PUDO, None
    ...
    try:
        raiz = tkinter.Tk()
        raiz.withdraw()
        # El selector sale DELANTE: nace sin ventana padre, y sin esto
        # Windows lo puede dejar detras de lo que hubiera abierto -- un
        # dialogo que no se ve se lee igual que un programa colgado.
        raiz.attributes("-topmost", True)
        try:
            elegida = filedialog.askdirectory(
                parent=raiz, mustexist=True,
                title=_t("carcasa.elige_carpeta",
                         "Elige la carpeta en la que va a trabajar Jarvis"))
        finally:
            raiz.destroy()
    except Exception:  # noqa: BLE001
        return NO_SE_PUDO, None

    # `askdirectory` devuelve "" tanto si cierras el dialogo como si le
    # das a Cancelar. Las dos son la misma respuesta: dijo que no.
    if not elegida:
        return CANCELO, None
    return ELEGIDA, Path(elegida)
...
def main() -> int:
    ...
    if not crudo:
        # >>> AQUI SE PREGUNTA, QUE ES LO QUE FALTABA <<< Hasta el
        # 2026-09-10 esto era un callejon: un cuadro de "no ha podido
        # arrancar" que mandaba a la linea de ordenes. Medido ese dia,
        # le pasaba a CUALQUIERA que clonase el repositorio -- en una
        # config recien sembrada `sesion.carpeta` es "" y ni
        # `jarvis.bat` ni el acceso directo pasan carpeta --, o sea que
        # el primer doble clic de un usuario nuevo no arrancaba.
        # Ahora se elige aqui, con el selector de Windows, y se guarda:
        # la proxima vez ya no pregunta.
        que, elegida = pedir_carpeta()
        if que == ELEGIDA and elegida is not None:
            try:
                from nucleo.ajustes import guardar as _guardar

                _guardar({"sesion.carpeta": str(elegida)})
            except Exception as exc:  # noqa: BLE001
                # >>> SE ARRANCA IGUAL, Y SE DICE <<< Que no se haya
                # podido GUARDAR la eleccion no es razon para no
                # ABRIRLA: ya eligio. Lo que no se puede hacer es
                # callarlo, o la proxima vez vuelve a preguntar y
                # parecera que la pantalla se olvida de lo que le dices.
                # `print` y no un metodo del diario: con `pythonw` la
                # salida ya esta desviada ahi, y es como escribe el
                # resto de este archivo.
                print(f"  no se pudo guardar la carpeta elegida: {exc}")
                avisar_de_que_no_arranco(
                    "Se abre esta vez en la carpeta que elegiste, pero no "
                    "se ha podido recordar. Ponla en AJUSTES > Para "
                    "empezar > Carpeta de trabajo para no repetirlo.",
                    diario.archivo)
            crudo = str(elegida)
        elif que == CANCELO:
            # Dijo que no. No se le suelta un parrafo sobre lineas de
            # ordenes: se le dice como volver a abrirlo y ya.
            avisar_de_que_no_arranco(
                "No se eligio ninguna carpeta, asi que Jarvis no ha "
                "arrancado. Vuelve a abrirlo cuando quieras y te la "
                "preguntara otra vez.",
                diario.archivo)
            return 2
        else:
            # No se pudo ni preguntar. Este es el unico caso en el que
            # hace falta explicar el camino manual, y es el de antes.
            avisar_de_que_no_arranco(
                "No hay carpeta de trabajo todavia y no se ha podido "
                "preguntar por ella. Arranca una vez pasandola detras "
                "del comando, y luego dejala puesta en AJUSTES > Para "
                "empezar > Carpeta de trabajo.",
                diario.archivo)
            return 2
